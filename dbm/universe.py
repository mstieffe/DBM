...
class Universe():
    ...
    def __init__(self, cfg, path_dict, ff):
        ...
        try:
            self.kick_center = self.cfg.getfloat('universe', 'kick_center')
        except:
            self.kick_center = 0.0
        try:
            self.only_rm_currbead = self.cfg.getboolean('universe', 'only_rm_currbead')
        except:
            self.only_rm_currbead = False
        ...
        for mol in self.mols:
            ...
            cg_seq, dict_aa_seq_heavy, dict_aa_seq_hydrogens, dict_aa_predecessors = mol.aa_seq(order=self.order,
                                                                                              train=False, only_rm_currbead=self.only_rm_currbead)
        ...
        if self.kick_center > 0.0:
            for b in self.beads:
                b.center += np.random.normal(-self.kick_center, self.kick_center, 3)
