...
from dbm.torch_energy_tab import *
...
class GAN_seq():
    ...
    def __init__(self, device, cfg):
        ...
        self.max_width = self.cfg.getfloat('grid', 'length')/ 2.0
        ...
        # Model selection
        if cfg.get('model', 'model_type')  == "probabilistic":
            self.voxel = False
    ...
    def get_energies_from_grid(self, atom_grid, energy_ndx):
        ...
        if bond_ndx.size()[1]:
            if self.bond_pot == "threshold":
                b_energy = self.energy.bond_threshold(coords, bond_ndx)
            else:
        ...
        if angle_ndx.size()[1]:
            if self.angle_pot == "threshold":
                a_energy = self.energy.angle_threshold(coords, angle_ndx)
            else:
    ...
    def get_energies_from_coords_threshold(self, coords, energy_ndx):
    ...
    def get_energies_from_coords_threshold2(self, coords, energy_ndx):
    ...
    def get_energies_from_coords_test(self, coords, energy_ndx):

        bond_ndx, angle_ndx, dih_ndx, lj_ndx = energy_ndx

        if bond_ndx.size()[1]:
            b_energy = self.energy.bond_test(coords, bond_ndx)
        else:
            b_energy = torch.zeros([], dtype=torch.float32, device=self.device)
        if angle_ndx.size()[1]:
            a_energy = self.energy.angle_test(coords, angle_ndx)
        else:
            a_energy = torch.zeros([], dtype=torch.float32, device=self.device)
        d_energy = torch.zeros([], dtype=torch.float32, device=self.device)
        if lj_ndx.size()[1]:
    ...
    def energy_match_loss(self, real_coords, fake_coords, energy_ndx):
    ...
    def mse_feature_match(self, real_coords, fake_coords, energy_ndx):
        bond_ndx, angle_ndx, dih_ndx, lj_ndx = energy_ndx

        if bond_ndx.size()[1]:
            b_loss = self.energy.dis_mse(real_coords, fake_coords, bond_ndx)
        else:
            b_loss = torch.zeros([], dtype=torch.float32, device=self.device), torch.zeros([], dtype=torch.float32, device=self.device)
        if angle_ndx.size()[1]:
            a_loss = self.energy.angle_mse(real_coords, fake_coords, angle_ndx)
        else:
            a_loss = torch.zeros([], dtype=torch.float32, device=self.device), torch.zeros([], dtype=torch.float32, device=self.device)
        if lj_ndx.size()[1]:
            nb_loss = self.energy.nb_mse(real_coords, fake_coords, lj_ndx)
        else:
            nb_loss = torch.zeros([], dtype=torch.float32, device=self.device)

        b_loss = torch.mean(b_loss)
        a_loss = torch.mean(a_loss)
        nb_loss = torch.mean(nb_loss)

        return b_loss, a_loss, nb_loss

    def mse_feature_match_with_lj(self, real_coords, fake_coords, energy_ndx):
        bond_ndx, angle_ndx, dih_ndx, lj_ndx = energy_ndx
        ...
        if lj_ndx.size()[1]:
            if self.nb_pot == "lj":
                l1_energy = self.energy.lj(real_coords, lj_ndx)
                l2_energy = self.energy.lj(fake_coords, lj_ndx)
            else:
                l1_energy = self.energy.quadratic_repl(real_coords, lj_ndx)
                l2_energy = self.energy.quadratic_repl(fake_coords, lj_ndx)
            nb_loss = torch.mean(torch.abs(l1_energy - l2_energy))
    ...
    def train_step_gen(self, elems, initial, energy_ndx, backprop=True):
        ...
        for target_atom, target_type, aa_featvec, repl, mask in elems:
            ...
            if self.voxel:
                ...
                mu_logsigma = self.generator(z, target_type, c_fake)
                new_coord = self.sample(mu_logsigma)
                fake_atom = self.to_voxel(new_coord)
                new_coord = torch.clamp(new_coord, -self.max_width, self.max_width)
        ...
        if self.prior_mode == 'match' and self.prior_weight() > 0.0:
            b_loss, a_loss, d_loss, l_loss = self.energy_match_loss(real_aa_coords, fake_aa_coords, energy_ndx)
            energy_loss = (self.ratio_bonded_nonbonded*(b_loss + a_loss + d_loss) + l_loss) * self.prior_weight()
            g_loss = g_wass + energy_loss
        elif self.prior_mode == 'mse_feature_match' and self.prior_weight() > 0.0:
            b_loss, a_loss, nb_loss = self.mse_feature_match(real_aa_coords, fake_aa_coords, energy_ndx)
            energy_loss = (self.ratio_bonded_nonbonded*(b_loss + a_loss) + nb_loss) * self.prior_weight()
            g_loss = g_wass + energy_loss
        ...
        elif self.prior_mode == 'mse_feature_match_with_lj' and self.prior_weight() > 0.0:
            b_loss, a_loss, nb_loss = self.mse_feature_match_with_lj(real_aa_coords, fake_aa_coords, energy_ndx)
            energy_loss = (self.ratio_bonded_nonbonded*(b_loss + a_loss) + nb_loss) * self.prior_weight()
            g_loss =  g_wass + energy_loss
    ...
    def predict(self, elems, initial, energy_ndx, bs):
        ...
        for target_type, aa_featvec, repl in zip(*elems):
            ...
            generated_atoms_coords.append(torch.squeeze(new_coord))
        generated_atoms_coords = torch.stack(generated_atoms_coords, dim=1)
        ...
        if self.val_mode == 'critic':
            energy = c_loss
        elif self.val_mode == 'threshold':
            b_energy, a_energy, d_energy, l_energy = self.get_energies_from_coords_threshold(aa_coords, energy_ndx)
            energy = b_energy + a_energy + d_energy + l_energy
        elif self.val_mode == 'threshold_wo_nb':
            b_energy, a_energy, d_energy, l_energy = self.get_energies_from_coords_threshold(aa_coords, energy_ndx)
            energy = b_energy + a_energy + d_energy
        elif self.val_mode == 'min_test':
            b_energy, a_energy, d_energy, l_energy = self.get_energies_from_coords_test(aa_coords, energy_ndx)
            energy = b_energy + a_energy + d_energy
        else:
    ...
    def validate(self, samples_dir=None):
        ...
        try:
            skip_init = self.cfg.getboolean('validate', 'skip_init')
        except:
            skip_init = False
        ...
        try:
            ...
            for data_gen in data_generators:
                ...
                for d in data_gen:
                    ...
                    if self.val_mode in ['min', 'match', 'critic', 'threshold', 'threshold_wo_nb', 'min_test']:
                        ndx = energies.argmin()
            ...
            subdir = "ep" + str(self.epoch) + "_valbs" + str(self.val_bs) + "_gibbs" + str(self.n_gibbs) + self.prior_mode + self.val_mode +"ifr" +str(initialize_from_reference) +"si"+str(skip_init)
