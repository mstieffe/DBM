...
class GAN_vae():
    ...
    def validate(self, samples_dir=None):
        ...
        try:
            ...
            for data_gen in data_generators:
                ...
                for d in data_gen:
                    ...
                    if self.prior_mode in ['min', 'match', 'critic']:
                        ...
                        print(mus)
                        print("sigma")
                        print(torch.exp(mu_logsigma[:, 3:])/2)
                        ...
                        time_start = timer()
                        #if m >= start_em:
                        new_coords = self.minimize_energy(new_coords, torch.from_numpy(d['aa_pos']).to(self.device).float(), torch.from_numpy(d['repl']).to(self.device), energy_ndx1, 0.0001, 10, mus, sigmas)
            ...
            subdir = "ep" + str(self.epoch) + "_" + self.prior_mode + "_valbs" + str(val_bs)+ "_gibbs" + str(self.n_gibbs)
