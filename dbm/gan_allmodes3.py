...
class DS_seq(Dataset):
    ...
    def __getitem__(self, ndx):
        ...
        initial = (atom_grid, cg_features, d['featvec_intra'])
...
class GAN_seq():
    ...
    def __init__(self, device, cfg):
        ...
        try:
            self.n_inp_intra_crit = self.cfg.getint('model', 'n_inp_intra_crit')
        except:
            self.n_inp_intra_crit = 4
        ...
        if cfg.get('model', 'model_type') == "tiny":
            ...
            if cfg.getint('grid', 'resolution') == 8:
                self.critic = model.AtomCrit_tiny(in_channels=self.ff.n_channels+1,
                                                  ...
                                                  device=device)
                self.critic_intra = model.AtomCrit_tiny(in_channels=self.n_inp_intra_crit,
                                                  start_channels=self.cfg.getint('model', 'n_chns'),
                                                  fac=1, sn=self.cfg.getint('model', 'sn_crit'),
                                                  device=device)
        ...
        self.critic_intra.to(device=device)
        ...
        self.opt_critic_intra = Adam(self.critic_intra.parameters(), lr=0.0001, betas=(0, 0.9))
    ...
    def gradient_penalty_intra(self, real_data, fake_data):
        batch_size = real_data.size()[0]
        ...
        # Calculate probability of interpolated examples
        prob_interpolated = self.critic_intra(interpolated)
    ...
    def train(self):
        ...
        for epoch in epochs:
            ...
            for n in range(0, steps_per_epoch):
                ...
                c_loss, c_loss_intra = self.train_step_critic(real=target_batch, fake=gen_input_batch)
    ...
    def train_step_critic(self, real, fake):
        ...
        aa_grid_real, cg_features_real, featvec_intra_real = initial_real
        aa_grid_fake, cg_features_fake, featvec_intra_fake = initial_fake

        intra_features_real = self.featurize(aa_grid_real, featvec_intra_real)
        intra_features_fake = self.featurize(aa_grid_fake, featvec_intra_fake)
        ...
        for target_atom, target_type, aa_featvec, repl, mask in elems_fake:
            ...
            intra_features_fake = torch.cat([intra_features_fake, fake_atom], dim=1)
        ...
        for target_atom, target_type, aa_featvec, repl, mask in elems_real:
            ...
            intra_features_real = torch.cat([intra_features_real, target_atom[:, None, :, :, :]], dim=1)
        ...
        #train critic for intra contributions
        #print(intra_features_real.size())
        #print(intra_features_fake.size())

        critic_intra_fake = torch.squeeze(self.critic_intra(intra_features_fake))
        critic_intra_real = torch.squeeze(self.critic_intra(intra_features_real))
        #print(critic_intra_fake)
        #print(critic_intra_real)
        c_wass_intra = self.critic_loss(critic_intra_real, critic_intra_fake)
        c_eps_intra = self.epsilon_penalty(1e-3, critic_intra_real)
        c_loss_intra = c_wass_intra + c_eps_intra
        if self.use_gp:
            c_gp_intra = self.gradient_penalty_intra(intra_features_real, intra_features_fake)
            c_loss_intra += c_gp_intra
        self.opt_critic_intra.zero_grad()
        c_loss_intra.backward()
        self.opt_critic_intra.step()


        return c_loss.detach().cpu().numpy(), c_loss_intra.detach().cpu().numpy()
    ...
    def train_step_gen(self, real, fake, backprop=True):
        ...
        critic_intra_loss = self.generator_loss(self.critic_intra(intra_features_fake))
        g_wass += critic_intra_loss
        ...
        g_loss_dict["Generator/wasserstein_intra"] = critic_intra_loss.detach().cpu().numpy()
