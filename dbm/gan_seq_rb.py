...
class DS_seq(Dataset):
    ...
    def __getitem__(self, ndx):
        ...
        energy_ndx = (d['bonds_ndx'], d['angles_ndx'], d['dihs_ndx'], d['dihs_rb_ndx'], d['ljs_ndx'])
...
class GAN_seq():
    ...
    def __init__(self, device, cfg):
        ...
        self.gauss_hist_bond = GaussianHistogram_Dis(bins=64, min=0.0, max=0.4, sigma=0.001, ff=self.ff, device=device)
        ...
        self.gauss_hist_nb = GaussianHistogram_LJ(bins=64, min=0.0, max=2.0, sigma=0.1, ff=self.ff, device=device)
    ...
    def get_energies_from_grid(self, atom_grid, energy_ndx):
        ...
        bond_ndx, angle_ndx, dih_ndx, dih_rb_ndx, lj_ndx = energy_ndx
        if bond_ndx.size()[1]:
        ...
        if dih_rb_ndx.size()[1]:
            drb_energy = self.energy.dih_rb(coords, dih_rb_ndx)
        else:
            drb_energy = torch.zeros([], dtype=torch.float32, device=self.device)
        if lj_ndx.size()[1]:
            l_energy = self.energy.lj(coords, lj_ndx)
        else:
            l_energy = torch.zeros([], dtype=torch.float32, device=self.device)

        return b_energy, a_energy, d_energy, drb_energy, l_energy
    ...
    def energy_min_loss(self, atom_grid, energy_ndx):
        ...
        fb, fa, fd, fdrb, fl = self.get_energies_from_grid(atom_grid, energy_ndx)

        b_loss = torch.mean(fb)
        a_loss = torch.mean(fa)
        d_loss = torch.mean(fd)
        drb_loss = torch.mean(fdrb)
        l_loss = torch.mean(fl)

        return b_loss, a_loss, d_loss, drb_loss, l_loss

    def energy_match_loss(self, real_atom_grid, fake_atom_grid, energy_ndx):

        rb, ra, rd, rdrb, rl = self.get_energies_from_grid(real_atom_grid, energy_ndx)
        fb, fa, fd, fdrb, fl = self.get_energies_from_grid(fake_atom_grid, energy_ndx)

        b_loss = torch.mean(torch.abs(rb - fb))
        a_loss = torch.mean(torch.abs(ra - fa))
        d_loss = torch.mean(torch.abs(rd - fd))
        drb_loss = torch.mean(torch.abs(rdrb - fdrb))
        l_loss = torch.mean(torch.abs(rl - fl))

        return b_loss, a_loss, d_loss, drb_loss, l_loss, torch.mean(fb), torch.mean(fa), torch.mean(fd), torch.mean(fdrb), torch.mean(fl)
                ...
                if n == n_critic:
                    ...
                    tqdm_train_iterator.set_description('D: {}, G: {}, {}, {}, {}, {}, {}, {}, {}'.format(c_loss,
                                                                                   ...
                                                                                   g_loss_dict['Generator/energy_dih'],
                                                                                    g_loss_dict['Generator/energy_dih_rb'],
    ...
    def train_step_gen(self, elems, initial, energy_ndx, backprop=True):
        ...
        if self.prior_mode == 'match':
            b_loss, a_loss, d_loss, drb_loss, l_loss, b_energy, a_energy, d_energy, drb_energy, l_energy = self.energy_match_loss(real_atom_grid, fake_atom_grid, energy_ndx)
            energy_loss = (self.ratio_bonded_nonbonded*(b_loss + a_loss + d_loss + drb_loss) + l_loss) * self.prior_weight()
            g_loss = g_wass + energy_loss
        elif self.prior_mode == 'min':
            b_energy, a_energy, d_energy, drb_energy, l_energy = self.energy_min_loss(fake_atom_grid, energy_ndx)
            energy_loss = (self.ratio_bonded_nonbonded*(b_energy + a_energy + d_energy + drb_energy) + l_energy) * self.prior_weight()
            g_loss = g_wass + energy_loss
        ...
        g_loss_dict["Generator/energy_dih_rb"] = drb_energy.detach().cpu().numpy()
    ...
    def predict(self, elems, initial, energy_ndx, bs):
        ...
        if self.prior_mode == 'critic':
            ...
            b_energy, a_energy, d_energy, drb_energy, l_energy = self.get_energies_from_grid(aa_grid, energy_ndx)
            energy = b_energy + a_energy + d_energy + drb_energy + l_energy
    ...
    def validate(self, samples_dir=None):
        ...
        if self.cfg.getboolean('training', 'init_intra'):
            data_generators.append(iter(Recurrent_Generator(self.data, hydrogens=False, gibbs=False, train=False, rand_rot=False, pad_seq=False, ref_pos=False, inter=False)))
            if self.cfg.getboolean('training', 'hydrogens'):
                data_generators.append(iter(Recurrent_Generator(self.data, hydrogens=True, gibbs=False, train=False, rand_rot=False,pad_seq=False, ref_pos=False, inter=False)))
        ...
        try:
            ...
            for data_gen in data_generators:
                ...
                for d in data_gen:
                    ...
                    if self.prior_mode in ['min', 'match', 'critic']:
                        ndx = energies.argmin()
                        new_coords = torch.matmul(new_coords[ndx], rot_mtxs_transposed[ndx])
                    elif self.prior_mode == "avg":
                        new_coords = torch.matmul(new_coords, rot_mtxs_transposed)
                        new_coords = torch.sum(new_coords, 0) / val_bs
                    elif self.prior_mode == "EM":
                        #new_coords = torch.matmul(new_coords, rot_mtxs_transposed)
                        #new_coords = torch.sum(new_coords, 0) / val_bs
                        ndx = energies.argmin()
                        new_coords = torch.matmul(new_coords[ndx], rot_mtxs_transposed[ndx])
                        new_coords = self.minimize_energy(new_coords, torch.from_numpy(d['aa_pos']).to(self.device).float(), torch.from_numpy(d['repl']).to(self.device), energy_ndx1, 1E-8, 20)
            ...
            subdir="ep" + str(self.epoch) + "_valbs" + str(val_bs)+ "_gibbs" + str(self.n_gibbs) + self.prior_mode
