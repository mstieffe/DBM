...
class Stats():
    ...
    def make_histo(self, values, n_bins=80, low=0.0, high=0.2):
        if values.any() and not np.any(np.isnan(values)):
    ...
    def bond_dstr(self, bond_name, samples, n_bins=80, ref=False):
        ...
        for sample in samples:
            ...
            for mol in sample.mols:
                if ref:
                    pos1 += [bond.atoms[0].ref_pos + bond.atoms[0].bead.center for bond in mol.bonds if
                             bond.type.name == bond_name]
                    pos2 += [bond.atoms[1].ref_pos + bond.atoms[1].bead.center for bond in mol.bonds if
                             bond.type.name == bond_name]
                else:
                    pos1 += [bond.atoms[0].pos + bond.atoms[0].bead.center for bond in mol.bonds if
                             bond.type.name == bond_name]
                    pos2 += [bond.atoms[1].pos + bond.atoms[1].bead.center for bond in mol.bonds if
                             bond.type.name == bond_name]
            if pos1:
    ...
    def angle_dstr(self, angle_name, samples, n_bins=80, ref=False):
        ...
        for sample in samples:
            ...
            for mol in sample.mols:
                if ref:
                    pos1 += [angle.atoms[0].ref_pos + angle.atoms[0].bead.center for angle in mol.angles if
                             angle.type.name == angle_name]
                    pos2 += [angle.atoms[1].ref_pos + angle.atoms[1].bead.center for angle in mol.angles if
                             angle.type.name == angle_name]
                    pos3 += [angle.atoms[2].ref_pos + angle.atoms[2].bead.center for angle in mol.angles if
                             angle.type.name == angle_name]

                else:
                    pos1 += [angle.atoms[0].pos + angle.atoms[0].bead.center for angle in mol.angles if
                             angle.type.name == angle_name]
                    pos2 += [angle.atoms[1].pos + angle.atoms[1].bead.center for angle in mol.angles if
                             angle.type.name == angle_name]
                    pos3 += [angle.atoms[2].pos + angle.atoms[2].bead.center for angle in mol.angles if
                             angle.type.name == angle_name]
            if pos1 != []:
    ...
    def dih_dstr(self, dih_name, samples, n_bins=80, low=0.0, high=360., ref=False):
        ...
        for sample in samples:
            ...
            for mol in sample.mols:
                if ref:
                    pos1 += [dih.atoms[0].ref_pos + dih.atoms[0].bead.center for dih in mol.dihs if
                             dih.type.name == dih_name]
                    pos2 += [dih.atoms[1].ref_pos + dih.atoms[1].bead.center for dih in mol.dihs if
                             dih.type.name == dih_name]
                    pos3 += [dih.atoms[2].ref_pos + dih.atoms[2].bead.center for dih in mol.dihs if
                             dih.type.name == dih_name]
                    pos4 += [dih.atoms[3].ref_pos + dih.atoms[3].bead.center for dih in mol.dihs if
                             dih.type.name == dih_name]

                else:
                    pos1 += [dih.atoms[0].pos + dih.atoms[0].bead.center for dih in mol.dihs if dih.type.name == dih_name]
                    pos2 += [dih.atoms[1].pos + dih.atoms[1].bead.center for dih in mol.dihs if dih.type.name == dih_name]
                    pos3 += [dih.atoms[2].pos + dih.atoms[2].bead.center for dih in mol.dihs if dih.type.name == dih_name]
                    pos4 += [dih.atoms[3].pos + dih.atoms[3].bead.center for dih in mol.dihs if dih.type.name == dih_name]
            if pos1 != []:
    ...
    def rdf(self, samples, n_bins=40, species=None, ref=False, excl=3, n_max=10000):
        ...
        for sample in samples:
            ...
            if ref:
                x = np.array([sample.box.move_inside(a.ref_pos + a.bead.center) for a in atoms])
            else:
