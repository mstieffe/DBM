...
class DS_seq(Dataset):
    def __init__(self, data, cfg, train=True):
        ...
        self.generator = Recurrent_Generator(data, hydrogens=self.hydrogens, gibbs=self.gibbs, train=train, rand_rot=False, inter=self.inter_condition)
...
class GAN_seq():
    ...
    def prepare_condition(self, fake_atom_grid, real_atom_grid, aa_featvec, bead_features):
        ...
        if self.cg_condition:
            c_fake = c_fake + bead_features
            c_real = c_real + bead_features
    ...
    def validate(self, samples_dir=None):
        ...
        data_generators = [Recurrent_Generator(self.data, hydrogens=self.hydrogens, gibbs=self.gibbs, train=False, rand_rot=False, inter=self.inter_condition, ref_pos=False)]

        if self.gibbs:
            for m in range(self.n_gibbs -1):
                data_generators.append(Recurrent_Generator(self.data, hydrogens=self.hydrogens, gibbs=self.gibbs, train=False, rand_rot=False, inter=self.inter_condition, ref_pos=False))
