...
class Recurrent_Generator2():

    def __init__(self, data, samples='val', hydrogens=False, gibbs=False, rand_rot=False, pad_seq=True, ref_pos=True, inter=True):
        ...
        if samples == 'target':
            self.samples = self.data.samples_target
            self.train = True
        elif samples == 'val':
            self.samples = self.data.samples_val
            self.train = False
        elif samples == 'gen_input':
            self.samples = self.data.samples_gen_input
            self.train = True
        else:
            raise Exception('sample selection not understood. Choose either train, val oder gen_input')
    ...
    def __iter__(self):
        ...
        for sample in self.samples:
            ...
            for bead in bead_seq:
                ...
                d["featvec_intra"] = np.array(self.pad2d(loc_env.featvec_intra(), self.data.max['atoms_loc_env']), dtype=np.float32)
                ...
                for atom in atom_seq_dict[bead]:
                    ...
                    if self.gibbs:
                        ...
                        if self.inter:
                            ...
                            #atom_featvec = self.pad2d(aa_f.fv_init_intra, self.data.max['atoms_loc_env'])
