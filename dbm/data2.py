...
class Data():
    ...
    def __init__(self, cfg, save=True):
        ...
        self.dirs_target = [Path("./data/reference_snapshots/") / d.replace(" ", "") for d in cfg.get('data', 'target').split(",")]
        try:
            self.dirs_gen_input = [Path("./data/reference_snapshots/") / d.replace(" ", "") for d in cfg.get('data', 'gen_input').split(",")]
            print("will use different data for Generator input than target data")
        except:
            self.dirs_gen_input = []
        self.dirs_val = [Path("./data/reference_snapshots/") / d.replace(" ", "") for d in cfg.get('data', 'val').split(",")]
        self.dir_processed = Path("./data/processed")
        self.dir_processed.mkdir(exist_ok=True)

        #self.samples_train, self.samples_val = [], []
        self.dict_target, self.dict_gen_input, self.dict_val = {}, {}, {}
        for path in self.dirs_target:
            self.dict_target[path.stem] = self.get_samples(path, save=save)
        self.samples_target = list(itertools.chain.from_iterable(self.dict_target.values()))

        if self.dirs_gen_input:
            for path in self.dirs_gen_input:
                self.dict_gen_input[path.stem] = self.get_samples(path, save=save)
            self.samples_gen_input = list(itertools.chain.from_iterable(self.dict_gen_input.values()))
        else:
            self.samples_gen_input = []
    ...
    def get_max_dict(self):
        ...
        samples = self.samples_target + self.samples_val
