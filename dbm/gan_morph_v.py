...
from dbm.recurrent_morph_generator import Recurrent_Generator
...
class DS_seq(Dataset):
    ...
    def __getitem__(self, ndx):
        ...
        return elems, atom_grid, energy_ndx
...
class GAN_seq():
        ...
        if cfg.get('model', 'model_type') == "tiny":
            ...
            if cfg.getint('grid', 'resolution') == 8:
                ...
                self.generator = model.AtomGen_VAE(z_dim=self.z_and_label_dim,
                                                    ...
                                                    start_channels=self.cfg.getint('model', 'n_chns'),
                                                    width=self.cfg.getfloat('grid', 'length')/3.0,
                                                    fac=1,
    ...
    def train_step_critic(self, elems, initial):
        ...
        for target_atom, target_type, aa_featvec, repl, mask in elems:
            ...
            c_fake = self.featurize(fake_atom_grid, aa_featvec)
            c_real = self.featurize(real_atom_grid, aa_featvec)
            #c_fake, c_real = self.prepare_condition(fake_atom_grid, real_atom_grid, aa_featvec)
            z = torch.empty(
    ...
    def dstr(self, atom_grid, energy_ndx):
        ...
        nan5 = torch.sum(torch.isnan(coords))
        if nan5 > 0:
            print("NANs in coords!!!!")
        else:
            print("keine NANE in coords")
    ...
    def hook_fn_backward(self, grad):
        grad = torch.where(torch.isnan(grad) > 0, torch.zeros_like(grad).to(grad.device), grad)
        return grad
    ...
    def train_step_gen(self, elems, initial, energy_ndx, backprop=True):
        ...
        if self.prior_mode == 'match' and self.prior_weight() > 0.0:
            ...
            dstr_loss = self.prior_weight() * ((b_dstr_loss + a_dstr_loss + d_dstr_loss) * self.ratio_bonded_nonbonded + nb_dstr_loss)
    ...
    def predict(self, elems, aa_grid, energy_ndx, bs):
            ...
            for data_gen in data_generators:
                ...
                for d in data_gen:
                    ...
                    with torch.no_grad():
                        ...
                        new_coords, energies, mu_logsigmas = self.predict(elems, aa_grid, energy_ndx, val_bs)
                    ...
                    d['bead'].center = d['bead'].center + new_center / len(d['atom_seq'])
