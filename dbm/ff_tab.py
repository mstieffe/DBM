...
class Bond_Type():
    ...
    def __init__(self, name, channel, tab_file ):
        self.name = name
        self.channel = int(channel)
        self.tab_file = str(tab_file)
...
class LJ_Type():
    ...
    def __init__(self, atom_type1, atom_type2, channel, tab_file):
...
class FF():
    ...
    def __init__(self, file):
        ...
        for line in read_between("[lj_types]", "[/lj_types]", self.file):
            name1, name2, channel, tab_file = line.split()
            self.n_channels = max(self.n_channels, int(channel) +1)
            self.lj_types[(name1, name2)] = LJ_Type(self.atom_types[name1], self.atom_types[name2], channel, tab_file)
        ...
        for line in read_between("[bond_types]", "[/bond_types]", self.file):
            ...
            name = (name1, name2)
            self.n_channels = max(self.n_channels, int(channel) +1)
            self.bond_types[name] = Bond_Type(name, channel, tab_file)
        ...
        for line in read_between("[angle_types]", "[/angle_types]", self.file):
            name1, name2, name3, channel, tab_file = line.split()
            name = (name1, name2, name3)
            self.n_channels = max(self.n_channels, int(channel) +1)
            self.angle_types[name] = Angle_Type(name, channel, tab_file)
        ...
        for line in read_between("[dihedral_types]", "[/dihedral_types]", self.file):
            name1, name2, name3, name4, channel, tab_file = line.split()
            name = (name1, name2, name3, name4)
            self.n_channels = max(self.n_channels, int(channel) + 1)
            self.dih_types[name] = Dih_Type(name, channel, tab_file)
    ...
    def bond_params(self, decimals=3):
        params = []
        for bond_type in self.bond_types.values():
            if isinstance(bond_type, Bond_Type):
                data = np.loadtxt(bond_type.tab_file)
                bond_dict = dict([(round(float(d[0]), decimals), [float(d[1]), float(d[2])]) for d in data])
                params.append(bond_dict)
    ...
    def angle_params(self, decimals=0):
        params = []
        for angle_type in self.angle_types.values():
            if isinstance(angle_type, Angle_Type):
                data = np.loadtxt(angle_type.tab_file)
                angle_dict = dict([(round(float(d[0]), decimals), [float(d[1]), float(d[2])]) for d in data])
                params.append(angle_dict)
        params.append({0.0: 0.0}) #dummie for padding..
        return np.array(params)
    ...
    def dih_params(self, decimals=0):
        params = []
        for dih_type in self.dih_types.values():
            if isinstance(dih_type, Dih_Type):
                data = np.loadtxt(dih_type.tab_file)
                dih_dict = dict([(round(float(d[0]), decimals), [float(d[1]), float(d[2])]) for d in data])
                params.append(dih_dict)
        params.append({0.0: 0.0}) #dummie for padding..
        return np.array(params)
    ...
    def lj_params(self, decimals=2):
        params = []
        for lj_type in self.lj_types.values():
            if isinstance(lj_type, LJ_Type):
                data = np.loadtxt(lj_type.tab_file)
                lj_dict = dict([(round(float(d[0]), decimals), [float(d[1]), float(d[2])]) for d in data])
                params.append(lj_dict)
        params.append({0.0: 0.0}) #dummie for padding..
        return np.array(params)

    def bond_lookups(self):
        dicts, d_0, delta = [], [], []
        for bond_type in self.bond_types.values():
            if isinstance(bond_type, Bond_Type):
                data = np.loadtxt(bond_type.tab_file)
                dicts.append(np.concatenate((data[:,1:], [[0.0, 0.0]]), 0))
                d_0.append(float(data[0,0]))
                delta.append(float(data[1,0])-float(data[0,0]))
        #params.append({0.0: 0.0}) #dummie for padding..
        return np.array(dicts), np.array(d_0), np.array(delta)

    def angle_lookups(self):
        dicts, d_0, delta = [], [], []
        for angle_type in self.angle_types.values():
            if isinstance(angle_type, Angle_Type):
                data = np.loadtxt(angle_type.tab_file)
