...
class Universe():
    ...
    def __init__(self, cfg, path_dict, ff):
        ...
        self.subbox_dict_cg = self.box.empty_subbox_dict()
        self.subbox_dict_aa = self.box.empty_subbox_dict()
        ...
        for res in cg.topology.residues:
            ...
            for bead in res.atoms:
                ...
                self.subbox_dict_cg[self.box.subbox(pos)].append(beads[-1])

                #print(self.box.subbox(beads[-1].center)[0])
            ...
            for line in read_between("[map]", "[/map]", map_file):
                ...
                if path_dict['aa_path']:
                    pos = self.box.move_inside(aa.xyz[0, atoms[-1].index])
                    atoms[-1].ref_pos = self.box.diff_vec(pos)
                    self.subbox_dict_aa[self.box.subbox(pos)].append(atoms[-1])
            ...
            if self.align:
                for line in read_between("[align_cg]", "[/align_cg]", map_file):
                    b_index, fp_index = line.split()
                    if int(b_index) > len(self.mols[-1].beads) or int(fp_index) > len(self.mols[-1].beads):
                        raise Exception('Indices in algn_cg section do not match the molecular structure!')
                    self.mols[-1].beads[int(b_index) - 1].fp = self.mols[-1].beads[int(fp_index) - 1]

                for line in read_between("[align_aa]", "[/align_aa]", map_file):
                    a_index, fp_index = line.split()
                    if int(a_index) > len(self.mols[-1].atoms) or int(fp_index) > len(self.mols[-1].atoms):
                        raise Exception('Indices in algn_aa section do not match the molecular structure!')
                    self.mols[-1].atoms[int(a_index) - 1].fp = self.mols[-1].atoms[int(fp_index) - 1]
    ...
    def get_loc_beads(self, bead):
        ...
        nn_beads = []
        for sb in subboxes:
            nn_beads += self.subbox_dict_cg[sb]
    ...
    def get_loc_atoms(self, atom):
        subboxes = self.box.nn_subboxes(atom.subbox)
        #print(bead.subbox)
        #print(subboxes)
        nn_atoms = []
        for sb in subboxes:
            nn_atoms += self.subbox_dict_aa[sb]
        centered_positions = np.array([a.ref_pos for a in nn_atoms]) - a.center
        centered_positions = np.array([self.box.diff_vec(pos) for pos in centered_positions])
        #centered_positions = self.box.pbc_diff_vec_batch(np.array(centered_positions))
        centered_positions_sq = [r[0] * r[0] + r[1] * r[1] + r[2] * r[2] for r in centered_positions]
