...
class DS_seq(Dataset):
    def __init__(self, data, cfg, samples='val'):
        ...
        #generators.append(Recurrent_Generator2(data, samples=samples, hydrogens=False, gibbs=False, rand_rot=False, inter=True))
        generators.append(Recurrent_Generator2(data, samples=samples, hydrogens=False, gibbs=True,
                                              rand_rot=False, inter=True))
        if cfg.getboolean('training', 'hydrogens'):
            ...
            #generators.append(Recurrent_Generator2(data, samples=samples, hydrogens=True, gibbs=False, rand_rot=False, inter=True))
            generators.append(Recurrent_Generator2(data, samples=samples, hydrogens=True, gibbs=True,
                                                   rand_rot=False, inter=True))
...
class GAN_seq():
    ...
    def train_step_critic(self, real, fake):
        ...
        aa_grid_real, cg_features_real, _ = initial_real
        aa_grid_fake, cg_features_fake, _ = initial_fake
        ...
        for target_atom, target_type, aa_featvec, repl, mask in elems_fake:
            ...
            c_fake = self.prepare_condition(aa_grid_fake, aa_featvec, cg_features_fake)
            z = torch.empty(
            ...
            fake_data.append(torch.cat([fake_atom, c_fake], dim=1))
    ...
    def train_step_gen(self, real, fake, backprop=True):
        ...
        aa_grid_real, cg_features_real, aa_coords_real = initial_real
        aa_grid_fake, cg_features_fake, aa_coords_fake = initial_fake
        ...
        for target_atom, target_type, aa_featvec, repl, mask in elems_fake:
            ...
            aa_coords_fake = torch.where(repl[:,:,None], aa_coords_fake, new_coord)
        ...
        g_loss_dict = {}
        b_energy, a_energy, d_energy, l_energy = self.energy_min_loss(aa_coords_fake, energy_ndx_fake)
        ...
        if self.prior_mode == 'match' and self.prior_weight() > 0.0:
            ...
            b_loss, a_loss, nb_loss = self.mse_feature_match(aa_coords_real, aa_coords_fake, energy_ndx)
            ...
            b_loss, a_loss, d_loss = self.dstr_loss(aa_grid_real, aa_grid_fake, energy_ndx)
    ...
    def validate(self, samples_dir=None):
        ...
        """
        data_generators.append(iter(
            Recurrent_Generator2(self.data, samples='val', hydrogens=False, gibbs=False,
                                 rand_rot=False, inter=True, ref_pos=False)))
        if self.cfg.getboolean('training', 'hydrogens'):
            data_generators.append(iter(
                Recurrent_Generator2(self.data, samples='val', hydrogens=True, gibbs=False,
                                     rand_rot=False, inter=True, ref_pos=False)))
        """

        for m in range(self.n_gibbs):
            data_generators.append(iter(
                Recurrent_Generator2(self.data, samples='val', hydrogens=False, gibbs=True,
                                     rand_rot=False, inter=True, ref_pos=False)))
            if self.cfg.getboolean('training', 'hydrogens'):
                data_generators.append(iter(
                    Recurrent_Generator2(self.data, samples='val', hydrogens=True, gibbs=True,
                                         rand_rot=False, inter=True, ref_pos=False)))
        m = 0
        try:
            self.generator.eval()
            ...
            for data_gen in data_generators:
                ...
                for d in data_gen:
                    ...
                    if self.val_mode in ['min', 'match', 'critic', 'threshold']:
                        ndx = energies.argmin()
            ...
            subdir = "ep" + str(self.epoch) + "_valbs" + str(self.val_bs) + "_gibbs" + str(self.n_gibbs) + self.prior_mode + self.val_mode + "ifr" + str(initialize_from_reference)
