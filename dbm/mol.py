...
class Atom():
    ...
    index = 0
    mol_index = 0
    def __init__(self, bead, mol, type, ref_pos=None, fp=None):
...
class Mol():
    ...
    def add_fp_aa(self, bead_ndx, fp_ndx1, fp_ndx2):
        self.fp[self.beads[bead_ndx]] = (self.atoms[fp_ndx1], self.atoms[fp_ndx2])
    ...
    def aa_seq(self, order="dfs", train=True, only_rm_currbead=False):
        ...
        for bead, predecessor_beads in cg_seq:
            ...
            for n in range(0, len(atom_seq)):
                if only_rm_currbead:
                    beads_except_curr = list(set(bead.mol.beads) - set(bead.atoms))
                    atoms_except_curr = list(itertools.chain.from_iterable([b.atoms for b in set(beads_except_curr)]))
                    atoms_except_curr = [a for a in atoms_except_curr if a.type.mass >= 2.0]
                    atom_predecessors_dict[atom_seq[n]] = atoms_except_curr + atom_seq[:n]
                else:
